...
straight_roads <- straight_roads %>% 
  # filter(str_detect(name, "(?i)the straight|strait|strate|straight"))
  filter(str_detect(name, "(?i)the strait|the street|roman road|straight road|the straight|streht"))
par(mar = rep(0.1, 4))
...
Modern road data (in blue) for `r paste0(regions, collapse=", ")` from Open Street Map, matched by name including "(?i)the strait|the street|roman road|straight road|the straight|streht".
