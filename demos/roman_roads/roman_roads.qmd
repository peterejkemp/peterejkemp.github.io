...
straight_roads <- straight_roads %>% 
  ...
  filter(str_detect(name, "(?i)the strait|the causeway|straight lane|streat lane|roman lane|straight lane|the street|roman road|roman bank|straight road|the straight|streht|street road|street lane|long causeway|east|west|north|south"))
...
leaflet() %>%
  ...
  addPolylines(data = straight_roads %>% 
                 filter(!grepl("Straight|Streat|Bank|Roman Road|The Causeway|The Street|East Street|West Street|North Street|South Street",name)), 
              color = "blue", 
              weight = 2,
              label = ~name,
              group="other_roads") %>%
