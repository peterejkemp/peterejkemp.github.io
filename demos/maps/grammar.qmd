---
title: "Grammar school Free School Meal students in England 2012-22"
format:
  html:
    toc: true
  # pdf_document:
...
_Expected_ values for the school given as numbers (e.g. `120` and `118`), if the school macthed the FSM figure for the Local Authority and Region. Percentages given are the percentage of students in the Local Authority and Region who are on free school meals
...
The things that predicts the percentage of FSM students in a school are the percentage of FSM students in a LA and the progress 8 score of school. The higher the progress 8 (`P8MEA`), the _lower_ the percentage of FSM students in a school:

> Sch FSM % ~ Gender + LA FSM % + Region FSM % + Progress 8

```{r stats}
### STATS
lm(data = grms %>% filter(Year %in% max(Year)), 
   `Sch FSM %` ~ Gender + `LA FSM %` + `Region FSM %` + `Progress 8`) %>% 
  summary() %>%
  # tidy() %>%
  tbl_regression() %>%
